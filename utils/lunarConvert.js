...
var Lunar = {
  ...
  getLunarDay(solarDate) {
    if (!(solarDate instanceof Date)) {
      if (typeof solarDate !== 'string') {
        console.error('[error]GetLunarDay failed, passed in solarDate is neither instance of Date, nor date string:' + solarDate);
        throw new Error('invalid solarDate passed in(must be either string or Date)');
      }
      let arr = solarDate.split('-');
      if (arr.length != 3) {
        console.error('[error]GetLunarDay failed, invalid solarDate: ' + solarDate);
  ...
  toFullLunar(year, month, day) {
    // [1891, 1, 1, '辛卯', '兔', '正月', '初一']
    const arr = Lunar.toLunar(year, month, day);
...
if (__DEV__) {
  ...
  for (let i = 0; i < total; i++) {
    ...
    const fullLunar = Lunar.getLunarDay(solar);
    const {
      fullLunarDayString,
      lunaryDay,
    ...
    if (!isLunarDayCorrect) {
      console.warn(`lunar day is wrong: ${fullLunarDayString} !== ${expectLunarDay}`);
    }
    if (!isFestivalCorrect) {
      console.warn(`festival is wrong: ${expFestival} !== ${resFestival}`);
    }
    console.error(`case ${i}/${total - 1} failed.
      solarDay: ${solar}
  ...
  if (0 < failedCount) {
    console.error(`Lunar Calculation Cases Failed, only passed: ${total - failedCount} of ${total}`);
  } else {
    console.info(`All lunar calculation cases(${total}) passed!`);
  }
...
module.exports = {
  toFullLunar: Lunar.toFullLunar,
  getLunarDay: Lunar.getLunarDay,
};
